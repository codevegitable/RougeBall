...
const GUIDES = {
    ...
    boss5: {
        ...
        lines: () => {
            const b = state.boss;
            if (!b) return ["Boss 战开始！"];
            const isCore = b.name === "终焉审判";
            return [
                `${b.name} 降临！ — 四阶段聚合体`,
                "一阶·钢铁甲壳：冲锋跳砸，正面减伤、背击增伤",
                "二阶·腐化内核：召唤治疗花、腐化花与藤蔓",
                "三阶·蜂群武装：主炮激光蓄力，预警期击打可打断",
                "终阶·司祭之眼：诅咒祭坛 + 可打断的蓄力大招",
                isCore ? "弹幕：扇形、追踪、分裂、环形轮番倾泻" : "弹幕：螺旋、追踪、波浪、环形轮番倾泻",
            ];
        },
    },
};
